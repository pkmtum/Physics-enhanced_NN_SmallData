...
def get_general_odeint_dataset(X0, t):
    ...
    def general_analytical_odeint(x0, t):
        return odeint(analytical_dynamics, x0, t, rtol=1e-10, atol=1e-10)
...
x_train, xt_train = jax.device_get(get_general_odeint_dataset(X0, t))  # dynamics for first N time steps
x_test, xt_test = jax.device_get(get_general_odeint_dataset(X0, t_test))  # dynamics for next N time steps
...
def loss_fun(params, batch):
    ...
    preds = jax.vmap(
        partial(LNN_dynamics, learned_lagrangian(params))
    )(states)
...
pickle_name = path + "singlePendulum_LNN_2x32_Data4x16_params_for_loss_{}.pkl".format(val_loss)
...
plot_lagrangian_IdMap(xt_test, xt_pred, figname=path + "singlePendulum_LNN_2x32_Data4x16_IdMap.png")
